...
function IncidentUpdate() {
  ...
  React.useEffect(() => {
    ...
    if (Incident.data) {
      ...
      setRegion(Help.setText(i.region))
      setProvince(Help.setText(i.province))
  ...
  function submitForm(e) {
    ...
    const DATA = {
      ...
      region: Help.formInputText(region),
      province: Help.formInputText(province),
    ...
    React.useEffect(() => {
      if (map && Incident.data) {
        ...
        map.on('click', (e) => {
          new mapboxgl.Popup()
          .setLngLat(e.lngLat)
          .setHTML(`<p style="color: black;">${e.lngLat}</p>`)
          .addTo(map);
    ...
    <PageContent>
      <FadeAnimation>
        <Form status={status}>
          ...
          <div className="sub-section-header">2.4 Address and Location</div>
          <FormRow>
          <Field error={helper.region} label="Region" status={status}>
              <Select
                onChange={(e) => {
                  setBarangay('')
                  setMunicipality('')
                ...
                value={region}>
                <option value=""></option>
                {Address.Regions(region).sort().map((item, index) => (
          ...
          <Field error={helper.province} label="Province" status={status}>
              <Select
                onChange={(e) => {
                  setBarangay('')
                  setMunicipality('')
                ...
                value={province}>
                <option value=""></option>
                {Address.Provinces(region).map((item, index) => (
                  <option key={index} value={item}>
                    {item}
                  </option>
                ))}
            ...
            <Field error={helper.municipal} label="Municipality" status={status}>
              <Select
                ...
                value={municipality}>
                <option value=""></option>
                {Address.Municipalities(region, province).map((item, index) => (
                  <option key={index} value={item}>
                    {item}
                  </option>
                ))}
            ...
            <Field error={helper.barangay} label="Barangay" status={status}>
              <Select onChange={(e) => setBarangay(e.target.value)} value={barangay}>
                <option value=""></option>
                {Address.Barangays(region, province, municipality).map((item, index) => (
                  <option key={index} value={item}>
                    {item}
                  </option>
                ))}
          ...
          <FormRow>
            <div id="incident-map" className="map-container-incident-picker" />
          </FormRow>
          <div className="sub-section-header">2.5 Date and Time of Incident</div>
          <FormRow>
